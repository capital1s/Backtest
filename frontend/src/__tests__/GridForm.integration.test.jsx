...
describe('GridForm Integration - Multi-step and Edge Cases', () => {
  ...
  const alerts = screen.queryAllByRole('alert');
  expect(alerts.length).toBeGreaterThan(0);
  const found = alerts.some(a => /invalid|error|please fill in all fields/i.test(a.textContent));
  expect(found).toBe(true);
    });
  });

  it('handles duplicate rapid submissions (stress test)', async () => {
  global.fetch = vi.fn().mockResolvedValue({ ok: true, json: async () => ({ result: 'success', trades: [1], heldShares: [2], performance: { profit: 10 } }) });
    render(<GridForm {...mockProps} />);
  ...
  global.fetch = vi.fn()
    .mockResolvedValueOnce({ ok: false, status: 400, text: async () => 'Bad input!', error: 'Bad input!' })
  ...
  fireEvent.change(screen.getByRole('combobox', { name: /ticker/i }), { target: { value: 'AAPL' } });
    ...
    await waitFor(() => {
      ...
      if (alerts.length > 0) {
        const found = alerts.some(a => /backtest completed successfully|error|invalid|bad input/i.test(a.textContent));
        expect(found).toBe(true);
      }
    });
  });
  it('handles boundary numeric values', async () => {
  global.fetch = vi.fn().mockResolvedValue({ ok: true, json: async () => ({ result: 'success', trades: [1], heldShares: [2], performance: { profit: 0 } }) });
    render(<GridForm {...mockProps} />);
  fireEvent.change(screen.getByRole('combobox', { name: /ticker/i }), { target: { value: 'AAPL' } });
    fireEvent.change(screen.getByLabelText(/number of shares/i), { target: { value: '0' } });
    fireEvent.change(screen.getByLabelText(/grid up value/i), { target: { value: '0' } });
  ...
  global.fetch = vi.fn().mockResolvedValue({ ok: true, json: async () => ({ result: 'success', trades: [999999], heldShares: [888888], performance: { profit: 1000000 } }) });
    render(<GridForm {...mockProps} />);
  ...
  it('handles API timeout and error', async () => {
    global.fetch = vi.fn().mockImplementation(() => Promise.reject(new Error('Network error: timeout')));
    render(<GridForm {...mockProps} />);
  fireEvent.change(screen.getByRole('combobox', { name: /ticker/i }), { target: { value: 'AAPL' } });
    fireEvent.change(screen.getByLabelText(/number of shares/i), { target: { value: '10' } });
    fireEvent.change(screen.getByLabelText(/grid up value/i), { target: { value: '1.0' } });
    ...
    await waitFor(() => {
      ...
      if (alerts.length === 0) {
        const foundStatus = statuses.some(s => /error|timeout|network/i.test(s.textContent));
        if (!foundStatus) {
          screen.debug();
        }
        expect(foundStatus).toBe(true);
      } else {
        const found = alerts.some(a => /error|timeout|network/i.test(a.textContent));
        if (!found) {
          screen.debug();
        }
        expect(found).toBe(true);
  ...
  it('handles sequential valid and invalid submissions', async () => {
    global.fetch = vi.fn()
      .mockResolvedValueOnce({ ok: false, status: 400, text: async () => 'Invalid ticker symbol', error: 'Invalid ticker symbol' })
      .mockResolvedValueOnce({ ok: true, json: async () => ({ result: 'success', trades: [1], heldShares: [2], performance: { profit: 10 } }) });
    render(<GridForm {...mockProps} />);
  ...
  fireEvent.change(screen.getByRole('combobox', { name: /ticker/i }), { target: { value: 'AAPL$%' } });
    ...
    await waitFor(() => {
      const alerts = screen.queryAllByRole('alert');
      const found = alerts.some(a => /invalid|error|please fill in all fields|network/i.test(a.textContent.toLowerCase()));
      if (!found) {
        screen.debug();
      }
      expect(found).toBe(true);
  ...
  it('shows error then success then error in rapid sequence', async () => {
    global.fetch = vi.fn()
      .mockResolvedValueOnce({ ok: false, status: 400, text: async () => 'Please fill in all fields.', error: 'Please fill in all fields.' })
      .mockResolvedValueOnce({ ok: true, json: async () => ({ result: 'success', trades: [], heldShares: [], performance: null }) })
      .mockResolvedValueOnce({ ok: false, status: 500, text: async () => 'Server error!', error: 'Server error!' });
    render(<GridForm {...mockProps} />);
    // Error
      fireEvent.submit(screen.getByRole('form'));
      await waitFor(() => {
        ...
        const foundStatus = statuses.some(s => {
          ...
          return txt.includes('please fill in all fields') || txt.includes('network error') || txt.includes('server error');
        });
  ...
  fireEvent.change(screen.getByRole('combobox', { name: /ticker/i }), { target: { value: 'AAPL' } });
    ...
    await waitFor(() => {
      ...
      const foundStatus = statuses.some(s => {
        const txt = s.textContent.toLowerCase();
        return txt.includes('backtest completed successfully') || txt.includes('network error') || txt.includes('server error');
      });
      const foundAlert = alerts.some(a => {
        const txt = a.textContent.toLowerCase();
        return txt.includes('backtest completed successfully') || txt.includes('network error') || txt.includes('server error');
  ...
  fireEvent.change(screen.getByRole('combobox', { name: /ticker/i }), { target: { value: 'AAPL' } });
    ...
    await waitFor(() => {
      ...
      if (alerts.length === 0) {
        let foundStatus = statuses.some(s => /server error!|network error: no response from server/i.test(s.textContent));
        if (!foundStatus) {
          try {
            const hiddenContainer = screen.getByTestId('hidden-error-success');
            foundStatus = Array.from(hiddenContainer.querySelectorAll('span')).some(
              el => /server error!|network error: no response from server/i.test(el.textContent)
            );
          } catch {}
        }
        if (!foundStatus) {
        ...
        let found = alerts.some(a => /server error!|network error: no response from server/i.test(a.textContent));
        if (!found) {
          try {
            const hiddenContainer = screen.getByTestId('hidden-error-success');
  ...
  global.fetch = vi.fn().mockResolvedValue({ ok: true, json: async () => ({ result: 'success', trades: [], heldShares: [], performance: null }) });
    render(<GridForm {...mockProps} />);
  fireEvent.change(screen.getByRole('combobox', { name: /ticker/i }), { target: { value: 'AAPL' } });
    fireEvent.change(screen.getByLabelText(/number of shares/i), { target: { value: '10' } });
    fireEvent.change(screen.getByLabelText(/grid up value/i), { target: { value: '1.0' } });
