...
describe("App integration", () => {
  ...
  it("submits grid form and displays results", async () => {
    ...
    if (forms.length !== 1 || dropdowns.length !== 1) {
      console.error(
        `FAIL: Expected 1 form and 1 ticker dropdown, found forms=${forms.length}, dropdowns=${dropdowns.length}`,
      );
      ...
      async () => {
        ...
        setTimeout(() => {
          ...
          statuses.forEach((el, idx) =>
            console.log(`Status after submit[${idx}]:`, el.textContent),
          );
          alerts.forEach((el, idx) =>
            console.log(`Alert after submit[${idx}]:`, el.textContent),
          );
          if (hiddenContainer) {
            Array.from(hiddenContainer.querySelectorAll("span")).forEach(
              (el, idx) =>
                console.log(
                  `Hidden span after submit[${idx}]:`,
                  el.textContent,
                ),
            );
          }
          ...
          if (spinner) {
            console.log(
              "Spinner still present after submit:",
              spinner.textContent,
            );
          }
          if (!statuses.length && !alerts.length && !hiddenContainer) {
            console.log(
              "No status/alert/hidden container found after submit. Printing DOM:",
            );
            screen.debug();
          }
        ...
        await waitFor(
          () => {
            ...
            if (spinner) {
              console.error(
                "Spinner still present after 2s:",
                spinner.textContent,
              );
              throw new Error(
                "Spinner did not disappear in time. Possible API/mock issue.",
              );
            }
      ...
      it("shows error for missing fields", async () => {
        ...
        let tickerDropdowns = [];
        try {
          tickerDropdowns = screen.getAllByRole("combobox", {
            name: /Ticker/i,
          });
        } catch {
          const fallback = screen.queryAllByLabelText(/Ticker/i, {
            hidden: true,
          });
      ...
      it("shows error for invalid ticker", async () => {
        ...
        await userEvent.type(
          screen.getByLabelText(/Grid increment value/i),
          "0.1",
        );
        const button = screen.getByRole("button", { name: /Backtest/i });
        fireEvent.submit(button.closest("form"));
        await waitFor(
          () => {
            ...
            if (!found) {
              const hiddenContainer = screen.queryByTestId(
      ...
      it("shows error for invalid shares", async () => {
        ...
        if (tickerDropdowns.length === 0) {
          ...
          if (!found) {
            const alerts = screen.queryAllByRole("alert", { hidden: true });
            found = alerts.some((a) =>
              /error|invalid value for shares/i.test(a.textContent),
            );
          }
          if (!found) {
            const statuses = screen.queryAllByRole("status", { hidden: true });
            found = statuses.some((s) =>
              /error|invalid value for shares/i.test(s.textContent),
            );
          }
          expect(found).toBe(true);
          return;
      ...
      it("shows error for invalid grid up", async () => {
        ...
        if (tickerDropdowns.length === 0) {
          ...
          if (!found) {
            const alerts = screen.queryAllByRole("alert", { hidden: true });
            found = alerts.some((a) =>
              /error|invalid value for grid up/i.test(a.textContent),
            );
          }
          if (!found) {
            const statuses = screen.queryAllByRole("status", { hidden: true });
            found = statuses.some((s) =>
              /error|invalid value for grid up/i.test(s.textContent),
            );
          }
          expect(found).toBe(true);
          return;
      ...
      it("shows error for invalid grid down", async () => {
        ...
        if (tickerDropdowns.length === 0) {
          ...
          if (!found) {
            const alerts = screen.queryAllByRole("alert", { hidden: true });
            found = alerts.some((a) =>
              /invalid value for grid down/i.test(a.textContent),
            );
          }
          if (!found) {
            const statuses = screen.queryAllByRole("status", { hidden: true });
            found = statuses.some((s) =>
              /invalid value for grid down/i.test(s.textContent),
            );
          }
          expect(found).toBe(true);
          return;
        ...
        await waitFor(
          async () => {
            const hiddenContainer = await screen.findByTestId(
              "hidden-error-success",
              {},
              { timeout: 30000 },
            );
      ...
      it("shows error for invalid grid increment", async () => {
        ...
        if (tickerDropdowns.length === 0) {
          ...
          if (!found) {
            const alerts = screen.queryAllByRole("alert", { hidden: true });
            found = alerts.some((a) =>
              /invalid value for grid increment/i.test(a.textContent),
            );
          }
          if (!found) {
            const statuses = screen.queryAllByRole("status", { hidden: true });
            found = statuses.some((s) =>
              /invalid value for grid increment/i.test(s.textContent),
            );
          }
          expect(found).toBe(true);
          return;
        ...
        await waitFor(
          () => {
            return screen.findByText(
              /Invalid value for grid increment. Must be a positive number./i,
              {},
              { timeout: 30000 },
            );
          },
      ...
      it("shows error for negative/zero numeric values", async () => {
        ...
        await userEvent.clear(screen.getByLabelText(/Grid down value/i));
        await userEvent.type(
          screen.getByLabelText(/Grid down value/i),
          "0.0005",
        );
        await userEvent.clear(screen.getByLabelText(/Grid increment value/i));
        await userEvent.type(
      ...
      it("shows success message after successful backtest", async () => {
        // No local server.use; global handler only. This test will use the default MSW handler unless overridden globally.
        console.log(
          "[TEST] Starting: shows success message after successful backtest",
        );
        render(<App />);
        // ...existing code...
        let tickerDropdownsSuccess = [];
        try {
          tickerDropdownsSuccess = screen.getAllByRole("combobox", {
            name: /Ticker/i,
          });
        } catch {
          const fallback = screen.queryAllByLabelText(/Ticker/i, {
        ...
        if (tickerDropdownsSuccess.length > 0) {
          ...
          expect(
            status &&
              /backtest completed successfully/i.test(status.textContent),
          ).toBe(true);
          return;
        }
        ...
        const gridIncrementInputs =
          screen.getAllByLabelText(/Grid increment value/i);
        ...
        const submitButtonSuccess = screen.getAllByRole("button", {
          name: /Backtest/i,
        })[0];
      ...
      setTimeout(() => {
        ...
        statusEls.forEach((el, idx) =>
          console.log(`Status after initial render[${idx}]:`, el.textContent),
        );
        const hiddenContainer = screen.queryByTestId("hidden-error-success");
        if (hiddenContainer) {
          Array.from(hiddenContainer.querySelectorAll("span")).forEach(
            (el, idx) =>
              console.log(
                `Hidden span after initial render[${idx}]:`,
                el.textContent,
              ),
          );
        }
      ...
      setTimeout(() => {
        const statusEls = screen.queryAllByRole("status");
        statusEls.forEach((el, idx) =>
          console.log(`Status after ticker select[${idx}]:`, el.textContent),
        );
        const hiddenContainer = screen.queryByTestId("hidden-error-success");
        if (hiddenContainer) {
          Array.from(hiddenContainer.querySelectorAll("span")).forEach(
            (el, idx) =>
              console.log(
                `Hidden span after ticker select[${idx}]:`,
                el.textContent,
              ),
          );
        }
      ...
      setTimeout(() => {
        const statusEls = screen.queryAllByRole("status");
        statusEls.forEach((el, idx) =>
          console.log(`Status before submit[${idx}]:`, el.textContent),
        );
        const hiddenContainer = screen.queryByTestId("hidden-error-success");
        if (hiddenContainer) {
          Array.from(hiddenContainer.querySelectorAll("span")).forEach(
            (el, idx) =>
              console.log(`Hidden span before submit[${idx}]:`, el.textContent),
          );
        }
      }, 1200);
      await userEvent.click(submitButtonSuccess);
      setTimeout(() => {
        ...
        if (hiddenContainer) {
          Array.from(hiddenContainer.querySelectorAll("span")).forEach(
            (el, idx) =>
              console.log(`Hidden span after submit[${idx}]:`, el.textContent),
          );
        }
      }, 1500);
      // Enhanced diagnostics after submission
      ...
      await waitFor(
        () => {
          ...
          const statuses = screen.queryAllByRole("status");
          const alerts = screen.getAllByRole("alert");
          let found = spans.some((el) =>
            /Backtest completed successfully/i.test(el.textContent),
          );
          if (!found)
            found = statuses.some((s) =>
              /Backtest completed successfully/i.test(s.textContent),
          ...
          if (!found)
            found = alerts.some((a) =>
              /Backtest completed successfully/i.test(a.textContent),
            );
          if (!found && hiddenContainer)
            found = /Backtest completed successfully/i.test(
              hiddenContainer.textContent,
            );
          if (!found) {
      ...
      it("shows error then success then error in rapid sequence", async () => {
        ...
        if (!tickerDropdowns[0]) {
          ...
          console.log(
            ...
            }, tickerDropdowns=${tickerDropdowns.length}`,
          );
          if (!tickerDropdowns[0]) {
        ...
        try {
          gridIncrementInput =
            screen.getAllByLabelText(/Grid increment value/i)[0];
        } catch {
        ...
        console.log(
          "Shares value:",
          screen.getAllByLabelText(/Number of shares/i)[0].value,
        );
        console.log(
          "Grid up value:",
          screen.getAllByLabelText(/Grid up value/i)[0].value,
        );
        console.log(
          "Grid down value:",
          screen.getAllByLabelText(/Grid down value/i)[0].value,
        );
        console.log(
          "Grid increment value:",
          screen.getAllByLabelText(/Grid increment value/i)[0].value,
        );
        await userEvent.click(button);
        ...
        await waitFor(
          () => {
            ...
            const alerts = screen.queryAllByRole("alert");
            found = alerts.some((a) =>
              /Please fill in all fields|error|no ticker selected/i.test(
                a.textContent,
              ),
            );
            if (!found) {
        ...
        await waitFor(
          () =>
            expect(
              screen.queryByRole("status", { name: /Running backtest/i }),
            ).toBeNull(),
          { timeout: 20000 },
        );
        ...
        await userEvent.type(
          screen.getAllByLabelText(/Number of shares/i)[0],
          "10",
        );
        await userEvent.clear(screen.getByLabelText(/Grid up value/i));
        await userEvent.type(screen.getByLabelText(/Grid up value/i), "1");
        ...
        try {
          ...
          gridIncrementInput2 =
            screen.queryByLabelText(/Grid increment value/i);
        }
        try {
        ...
        await waitFor(
          () => {
            ...
            const alerts = screen.queryAllByRole("alert");
            found = alerts.some((a) =>
              /Bad input|400|error/i.test(a.textContent),
            );
            if (!found) {
              const hiddenContainer = screen.queryByTestId(
                "hidden-error-success",
              ...
              found =
                hiddenContainer &&
                /Bad input|400|error/i.test(hiddenContainer.textContent);
            }
            expect(found).toBe(true);
          },
          { timeout: 20000 },
