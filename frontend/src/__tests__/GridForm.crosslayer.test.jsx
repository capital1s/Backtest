...
describe("GridForm Integration - Multi-step and Edge Cases", () => {
  ...
  it("handles concurrent submissions and race conditions", async () => {
    ...
    mockGlobalFetch(() =>
      Promise.resolve({
        ok: false,
        status: 400,
        json: async () => ({ error: "please fill in all fields" }),
      }),
    );
    await waitFor(() => {
      const statuses = screen.queryAllByRole("status");
  ...
  it("handles API timeout and error", async () => {
    ...
    await waitFor(() => {
      ...
      expect(statuses.length).toBeGreaterThan(0);
      const found = statuses.some((s) =>
        /completed successfully|rate limit exceeded|error/i.test(s.textContent),
      );
      expect(found).toBe(true);
    });
  });
  ...
  it("handles sequential valid and invalid submissions", async () => {
    ...
    await waitFor(
      () => {
        const alerts = screen.queryAllByRole("alert");
        const statuses = screen.queryAllByRole("status");
        let found = [...alerts, ...statuses].some((node) =>
          /invalid ticker symbol/i.test(node.textContent),
        );
        if (!found) {
          try {
